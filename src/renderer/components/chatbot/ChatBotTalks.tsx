...
const thinkingBtn = (name = '思考中') => (
  <Button type="primary" loading disabled>
    {name}
  </Button>
);
const suggestBtn = (i: string, name: string, callback: any) => (
  <Button
    key={i}
    style={{
      background: '#1677ff',
  ...
  <div style={{ margin: '2px 0' }} key={index}>
    {
      ...
      data.type == 'thinking' ? (
        ...
        <>
          {data.buttons && data.buttons.length > 0 && data.hi
            ? customizeRenderEmpty(data.hi)
            : ''}
          {data.buttons && data.buttons.length > 0
            ? Array.from(data.buttons, (button: any, i) =>
                suggestBtn(
                  `${i}`,
                  button.name,
...
class ChatBotTalks extends React.Component {
  ...
  _updateItems() {
    ...
    // 当this.props.items 为空
    let items: any = !(this.props.items && this.props.items.length > 0)
      ? defaultItems
      : [...this.props.items];
    ...
    items = Array.from(items, (item: any, index: number) => {
      ...
      const { html } = item;
      const buttons = Array.from(item.buttons || [], (button: any) => {
        if (button.from) {
          button.callback = () =>
            this.props.callback({ cmd: button.from, data: button.data });
  ...
  _go2Bottom() {
    const dom: any = this.contentDom.current;
    ...
    return (
      ...
      >
        {this.state.items &&
          ...
          Array.from(this.state.items, (item, index) =>
