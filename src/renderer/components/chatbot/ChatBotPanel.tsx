...
class ChatBotPanel extends React.Component {
  ...
  _createContentList(activeIndex: number) {
    ...
    const activeTabKey = tabList.filter(
      (tab: any) => activeIndex == tab.index
    )[0]?.key;
    ...
    for (const subject of tabList) {
      if (subject.index == -1) {
        ...
        const nds = Array.from(datas[subject.index + 1], (data: any, i) => {
          if (data && data.html) return { html: data.html, i };
        }).filter((n) => n);
        const listItem = nds.map((data: any) => (
          <div key={data.i} dangerouslySetInnerHTML={{ __html: data.html }} />
        ));
        contentList[subject.key] = (
          <Content display="flex" translate="no">
            <div>{listItem}</div>
          </Content>
        );
      }
  ...
  render() {
    const { contentList, activeTabKey } = this._createContentList(
    ...
    return (
      <Card
        ...
        style={{
          ...
          padding: this.state.fullscreen ? '0px' : '0px',
          borderRadius: 0,
          minWidth: '360px',
          display: 'flex',
          flexDirection: 'column',
        ...
        onTabChange={(key: string) => {
          this._switchSubject(
            this.state.tabList.filter((s: any) => s.key == key)[0].index
          );
        }}
      >
        {this.state.loading ? (
