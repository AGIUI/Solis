...
class ComboEditor extends React.Component {
  ...
  componentDidUpdate(prevProps: any, prevState: any) {
    ...
    const prevId = md5(JSON.stringify(prevProps.myPrompts));
        ...
        extra={
          <div>
            <Button
              ...
              onClick={() =>
                this.props.callback &&
                this.props.callback({ cmd: 'close-combo-editor' })
              }
      ...
      >
        ...
        >
                      ...
                      <a key={i}>
                          ...
                          actions={[
                            <Switch
                              ...
                              onClick={(checked: boolean, event: any) =>
                                this._editSwitch(checked, event, p)
                              }
                            />,
                            <Button
                              onClick={() =>
                                this._comboHandle(p, 'getPromptPage')
                              }
                            >
                              运行
                            </Button>,
                        ...
                        >
                          <Text style={{ fontWeight: 'bold' }}>
                            {p.tag}
                            {p.combo > 1 ? (
                              <Tag style={{ marginLeft: 10 }}>Combo</Tag>
                            ) : null}
                          </Text>
                        </List.Item>
                      </a>
          ...
          {this.state.myPrompts.filter((p: any) => p.owner !== 'official')
            .length > 0 ? (
            <List>
              {this.state.myPrompts
                .filter((p: any) => p.owner !== 'official')
                .map((p: any, i: number) => {
                  return (
                    <a key={i}>
                      <List.Item
        ...
        <Button
          size="large"
          type="primary"
          style={{ position: 'absolute', bottom: 30, width: '90%', left: '4%' }}
